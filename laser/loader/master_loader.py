...
from laser.loader.dataset.tu_dataset import TUDataset
from laser.loader.dataset.coco_superpixels import COCOSuperpixels
from laser.loader.dataset.malnet_tiny import MalNetTiny
from laser.loader.dataset.voc_superpixels import VOCSuperpixels
from laser.loader.split_generator import (prepare_splits,
                                             set_dataset_splits)
from laser.transform.posenc_stats import compute_posenc_stats
from laser.transform.transforms import (pre_transform_in_memory,
                                           typecast_x, concat_x_and_pos,
                                           clip_graphs_to_size)

from .dynamic_utils import get_dynamic_pre_transform
from ..rewiring.transform import TUDatasetTransform
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from laser.loader.dataset.pcqm4mv2_contact import \
    ...
    pre_transform = get_dynamic_pre_transform(name)

    dataset = PygPCQM4Mv2ContactDataset(dataset_dir, pre_transform=pre_transform)
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from laser.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from laser.loader.dataset.peptides_structural import \
    ...
    if dataset_type == 'functional':
        dataset = PeptidesFunctionalDataset(dataset_dir, pre_transform=pre_transform)
    elif dataset_type == 'structural':
        dataset = PeptidesStructuralDataset(dataset_dir, pre_transform=pre_transform)
    s_dict = dataset.get_idx_split()
    dataset.split_idxs = [s_dict[s] for s in ['train', 'val', 'test']]
    return dataset
...
def preformat_TUDataset(dataset_dir, name):
    ...
    if pre_transform is None and name in ["REDDIT-BINARY", "IMDB-BINARY", "MUTAG", "ENZYMES", "PROTEINS", "COLLAB"]:
        pre_transform = TUDatasetTransform(dataset=name).transform
